...
class IAViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val storageRepository: IStorageRepository =
        RepositoryProvider.provideStorageRepository(application)
    ...
    private val generarActividadesInteractivasUseCase by lazy {
        GenerarActividadesInteractivasUseCase(
            iaRepository
        )
    }
    ...
    private val cerrarSesionUseCase by lazy {
        cl.duocuc.aulaviva.domain.usecase.CerrarSesionUseCase(
            iaRepository
        )
    }
    ...
    fun analizarPdfConIA(nombre: String, pdfUrl: String?): LiveData<Result<String>> =
    ...
    fun resumirContenidoPdf(
        nombre: String,
        descripcion: String,
        pdfUrl: String?
    ): LiveData<Result<String>> = liveData(Dispatchers.IO) {
        try {
            val r = resumirContenidoPdfUseCase(nombre, descripcion, pdfUrl)
            emit(Result.success(r))
        } catch (e: Exception) {
            emit(Result.failure(e))
        }
    ...
    fun generarActividadesInteractivas(
        nombre: String,
        descripcion: String,
        pdfUrl: String?
    ): LiveData<Result<String>> = liveData(Dispatchers.IO) {
        try {
            val r = generarActividadesInteractivasUseCase(nombre, descripcion, pdfUrl)
            emit(Result.success(r))
        } catch (e: Exception) {
            emit(Result.failure(e))
        }
    ...
    fun explicarConceptosParaAlumno(
        nombre: String,
        descripcion: String,
        pdfUrl: String?
    ): LiveData<Result<String>> = liveData(Dispatchers.IO) {
        try {
            val r = explicarConceptosUseCase(nombre, descripcion, pdfUrl)
            emit(Result.success(r))
        } catch (e: Exception) {
            emit(Result.failure(e))
        }
    ...
    fun generarEjerciciosParaAlumno(
        nombre: String,
        descripcion: String,
        pdfUrl: String?
    ): LiveData<Result<String>> = liveData(Dispatchers.IO) {
    ...
    fun crearResumenEstudioParaAlumno(
        nombre: String,
        descripcion: String,
        pdfUrl: String?
    ): LiveData<Result<String>> = liveData(Dispatchers.IO) {
        try {
            val r = crearResumenEstudioUseCase(nombre, descripcion, pdfUrl)
            emit(Result.success(r))
        } catch (e: Exception) {
            emit(Result.failure(e))
        }
